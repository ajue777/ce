...
class v62H(IStrategy):    
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Previous ADX value
        last_candle = informative['adx']
        previous_candle = informative['adx'].shift(1)
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        if trade.nr_of_successful_entries >= 1:
            ...
            adx_condition = (last_candle['adx'] < previous_candle['adx']) and (dataframe['adx'] > 30)
            
            if (price_drop >= 0.1) and adx_condition:
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        """
        dataframe.loc[(dataframe['rsi'] < 20) & (dataframe['rsi_1h'] < 20), 'rsi_triggered'] = True

        # 可选：定义重置条件，例如 RSI > 70 时重置状态
        dataframe.loc[dataframe['rsi'] > 85, 'rsi_triggered'] = False

        dataframe.loc[
        (
            (dataframe['adx_prev_1h']) &

            (dataframe['rsi_triggered'])  # Make sure volume is not 0
        ),
        'enter_long',] = 1
