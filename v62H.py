...
class v62H(IStrategy):    
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if 'rsi_triggered' not in dataframe.columns:
            dataframe['rsi_triggered'] = False  # 仅在列不存在时初始化
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[(dataframe['rsi'] < 20) & (dataframe['rsi_1h'] < 25), 'rsi_triggered'] = True

        # 可选：定义重置条件，例如 RSI > 70 时重置状态
        dataframe.loc[dataframe['rsi'] > 70, 'rsi_triggered'] = False
