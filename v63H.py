...
class v63H(IStrategy):
    ...
    trailing_stop = True
    trailing_stop_positive = 0.05  # 利润达到 5% 时开始追踪
    trailing_stop_positive_offset = 0.10  # 利润达到 10% 时激活追踪止损
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['adx_weak'] = dataframe['adx'] < 25
        dataframe['adx_weak_confirmed'] = dataframe['adx_weak'] & dataframe['adx_weak'].shift(1)

        dataframe.loc[
            (dataframe['rsi'] > 80) | (dataframe['adx_weak_confirmed']),
            'exit_long'] = 1
        """
        dataframe['atr'] = ta.ATR(dataframe['high'], dataframe['low'], dataframe['close'], timeperiod=14)
        dataframe['profit_target'] = dataframe['close'].shift(1) + 2 * dataframe['atr']  # 入场点 + 2*ATR

        dataframe.loc[
            (dataframe['close'] >= dataframe['profit_target']) | (dataframe['rsi'] > 70),
            'exit_long'] = 1
        """
        dataframe['ma20'] = ta.SMA(dataframe, timeperiod=20)

        dataframe.loc[
            (dataframe['rsi'] > 80) | (dataframe['close'] < dataframe['ma20']),
            'exit_long'] = 1
        """    
        return dataframe
