...
class v65(IStrategy):    
    ...
    def __init__(self, config: dict) -> None:
            ...
            self.last_entry_price: Dict[int, float] = {}
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        # 入场/加仓条件
        last_entry_price = self.last_entry_price.get(trade.id, trade.open_rate)

        # 计算相对于上一次入场价格的利润
        profit_from_last_entry = (current_rate - last_entry_price) / last_entry_price
        
        should_adjust = dataframe['adx_prev'].iloc[-1] and dataframe['rsi_triggered'].iloc[-1]
        if should_adjust and profit_from_last_entry <= -0.06:
            filled_entries = trade.select_filled_orders(trade.entry_side)
            count_of_entries = trade.nr_of_successful_entries
            try:
                stake_amount = filled_entries[0].stake_amount_filled
                stake_amount = stake_amount * (1 + (count_of_entries * 0.05))
                self.last_entry_price[trade.id] = current_rate
                return stake_amount
